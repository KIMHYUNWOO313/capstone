# xai_explainer — 농산물 가격 예측 설명 (XAI)

Chronos2 LoRA(10일 확률예측)와 TimesFM 2.5(3일 점예측)가 산출한 농산물 도매가격 예측값을, GPT-4o가 한국어로 "왜 이 가격이 예측되었는가"를 설명해주는 패키지입니다.

본 폴더만 받으면 즉시 동작하도록 설계되었습니다 (모델 파일·환경 불필요).
